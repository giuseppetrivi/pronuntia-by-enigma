<?php

/** @var yii\web\View $this */
/** @var yii\bootstrap4\ActiveForm $form */
/** @var app\models\LoginForm $model */

use yii\bootstrap4\ActiveForm;
use yii\bootstrap4\Html;

$this->title = 'Prenota appuntamento';
$this->params['breadcrumbs'][] = ['label' => 'Appuntamenti', 'url' => ['caregiver/appointment']];
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="new-utente">
    <h1><?= Html::encode($this->title) ?></h1>

    <br>

    <?php 
        $form = ActiveForm::begin([
            'id' => 'utente-form',
            'action' => ['caregiver/appointment-form'],
            'method' => 'post',
            'layout' => 'horizontal',
            'fieldConfig' => [
                'template' => "{label}\n{input}\n{error}",
                'labelOptions' => ['class' => 'col-lg-2 col-form-label mr-lg-3'],
                'inputOptions' => ['class' => 'col-lg-3 form-control'],
                'errorOptions' => ['class' => 'col-lg-4 invalid-feedback'],
            ],
        ]); 

    ?>

        <?php
          $array_options = [];
          foreach ($logopedisti_salvati as $value) {
            $array_options[$value['id']] = $value['nome'] . ' ' . $value['cognome'];
          }
          echo $form->field($model, 'logopedista')->dropdownList($array_options);
        ?>

        <?= $form->field($model, 'data')->input('date') ?>

        <?= $form->field($model, 'ora')->input('time') ?>

        <?= $form->field($model, 'info')->textArea([
          'rows' => '3'
        ]) ?>

        <div class="form-group">
            <div class="offset-lg-1 col-lg-11">
                <?= Html::submitButton('Salva appuntamento', ['class' => 'btn btn-primary']) ?>
            </div>
        </div>

    <?php ActiveForm::end(); ?>

</div>
